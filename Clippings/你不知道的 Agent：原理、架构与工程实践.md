...
created: 2026-07-25
description: "研究完 Claude Code 的架构之后，发现自己对 Agent 底层的理解还不够深入，加上团队在 Agent 方向已经有不少业务落地，但一直缺少一份系统梳理，所以又补了一轮资料、开源实现和轻量实验，把控制流、上下文工程、工具设计、记忆、评测和安全这些问题重新串了一遍。"
tags:
  - "clippings"
---
<svg width="100%" height="200" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" role="img" aria-label="運籌帷幄之中，決勝千里之外"><path id="svg-title-path"></path><text font-size="42" font-weight="400" fill="#ffffff" x="50%" y="50%" text-anchor="middle" word-spacing="2.0" letter-spacing="4"><textPath xlink:href="#svg-title-path">運籌帷幄之中，決勝千里之外</textPath></text></svg>
...
![Skills 按需加载](https://gw.alipayobjects.com/zos/k/vw/WQUEG4.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![AI 工作模式变化](https://gw.alipayobjects.com/zos/k/gi/2.svg)
...
![多 Agent 协作协议](https://gw.alipayobjects.com/zos/k/g3/multi_agent_protocol.svg)
...
![多 Agent 幻觉放大](https://gw.alipayobjects.com/zos/k/z3/svgviewer-output%252520%281%29.svg)
...
![Single-turn vs Agent 评测对比：Single-turn 是 Prompt 进 LLM 出 Response 直接打分，Agent 则需要 Tools、Environment、Task 协同，Agent 多步调用工具并更新环境状态，最后验证环境实际结果而非只看输出文字](https://gw.alipayobjects.com/zos/k/3o/L7tEpR.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Agent 评测的组成部分：task、trial、grader、transcript、outcome、evaluation harness、agent harness 和 evaluation suite](https://gw.alipayobjects.com/zos/k/kd/YpOEZB.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
| ![调查：团队实际使用的评测方式，Offline evaluation on test sets 54.5%，Online evaluation on production data 44.8%，Not evaluating yet 22.8%](https://gw.alipayobjects.com/zos/k/ri/H5Cn8z.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp) | ![调查：常用评测指标，Internal human review/labelling 59.8%，LLM-as-judge 53.3%，Traditional ML/DS metrics 16.9%](https://gw.alipayobjects.com/zos/k/br/Al2yJz.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp) |
| --- | --- |
...
![Success rate vs infra error rate：横轴是评测容器的资源余量从 1x 到 Uncapped，蓝色是模型得分，红色是基础设施错误率，资源越受限红色越高蓝色越低](https://gw.alipayobjects.com/zos/k/sq/ERIBy8.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![两层可观测性](https://cdn.tw93.fun/pic/observability_two_layer59.svg)
...
![事件流可观测性](https://gw.alipayobjects.com/zos/k/wj/svgviewer-output.svg)
...
![OpenClaw 整体架构](https://gw.alipayobjects.com/zos/k/9k/openclaw.svg)
...
![系统提示分层叠加](https://gw.alipayobjects.com/zos/k/e9/svgviewer-output%252520%281%29.svg)
